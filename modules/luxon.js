...
function formatRelativeTime (unit, count, numeric = 'always', narrow = false) {
  ...
  const isInPast = Object.is (count, -0) || count < 0,
    ...
    lilUnits = units[unit],
    fmtUnit = narrow
      ? singular ? lilUnits[1] : lilUnits[2] || lilUnits[1]
      : singular ? units[unit][0] : unit;
...
function normalizeZone (input, defaultZone) {
  if (isUndefined (input) || input === null) {
    ...
    if (lowered === 'default') return defaultZone;
      ...
      return (
        FixedOffsetZone.parseSpecifier (lowered) || IANAZone.create (input)
      );
...
class Info {
  ...
  static months (
    ...
    return (locObj || Locale.create (locale, numberingSystem, outputCalendar))
      .months (length);
  }
  ...
  static weekdays (
    ...
    return (locObj || Locale.create (locale, numberingSystem, null))
      .weekdays (length);
  }
...
const hanidecChars = numberingSystems.hanidec
  .replace (/[\[|\]]/g, '')
  .split ('');
...
class DateTime {
  ...
  endOf (unit) {
    return this.isValid
      ? this.plus ({[unit]: 1}).startOf (unit).minus (1)
      : this;
  }
   ...
   */
  toSQLTime (
    {includeOffset = true, includeZone = false, includeOffsetSpace = true} = {}
  ) {
  ...
  toRelative (options = {}) {
    ...
    const base = options.base || DateTime.fromObject ({}, {zone: this.zone}),
      padding = options.padding
        ? this < base ? -options.padding : options.padding
        : 0;
